...
// Personalized & mobile-friendly edition
...
class BirthdayApp {
    ...
    setupCanvas() {
        ...
        // Design dimensions the heart points were created for
        const DESIGN_W = 670;
        const DESIGN_H = 625;

        // Scale factor for smaller screens (mobile)
        this.scale = Math.min(loveHeart.offsetWidth / DESIGN_W, loveHeart.offsetHeight / DESIGN_H);

        this.offsetX = loveHeart.offsetWidth / 2;
        this.offsetY = loveHeart.offsetHeight / 2 - 55 * this.scale;
    ...
    startGardenRenderLoop() {
        // Slightly faster render – shaved 10 ms off the default growSpeed
        const speed = Math.max(Garden.options.growSpeed - 10, 10);
        setInterval(() => {
            if (this.garden) this.garden.render();
        }, speed);
    }

    // ─── Heart drawing ────────────────────────────────────────────────────────
    startHeartAnimation() {
        // One point per tick at 6 ms → smooth gradual bloom
        const step     = 1;
        let   angle    = 0;
        const heart    = [];
        const interval = 6;
        ...
        this.heartAnimation = setInterval(() => {
            for (let s = 0; s < step; s++) {
                ...
                for (let i = 0; i < heart.length; i++) {
                    ...
                    if (d < Garden.options.bloomRadius.max * 1.3) {
                        draw = false;
                        break;
                    }
                }
    ...
    getHeartPoint(angle) {
        ...
        if (angle < xs.length && angle < ys.length) {
            return [this.offsetX + xs[angle] * this.scale, this.offsetY + ys[angle] * this.scale];
        }
    ...
    /**
     * Called every second while the "counting up" phase is active.
     * @param {number} totalSeconds – how many seconds since we started
     */
    showElapsedTime(totalSeconds) {
        const el = document.getElementById("elapseClock");
        if (!el) return;
        ...
        el.innerHTML =
            ...
            `<span class="digit">${pad(secs)}</span> seconds`;
    }

    /**
     * Freeze the clock at zero and show the "Hey it's now!" message,
     * then fade in the personal quote below.
     */
    showBirthdayNow() {
        const el = document.getElementById("elapseClock");
        if (el) {
            el.innerHTML =
                `<span class="digit">0</span> days ` +
                `<span class="digit">00</span> hours ` +
                `<span class="digit">00</span> minutes ` +
                `<span class="digit">00</span> seconds`;
        }

        // Show a quiet little "it's now" note
        let nowEl = document.getElementById('birthdayNow');
        if (!nowEl) {
            nowEl = document.createElement('div');
            nowEl.id = 'birthdayNow';
            nowEl.textContent = "— it's today ✨";
            const messages = document.getElementById('messages');
            if (messages) messages.appendChild(nowEl);
        }
        this.fadeIn(nowEl, 1200, () => {
            // After a short pause, show the love message
            setTimeout(() => this.showLoveU(), 800);
        });
    }

    // ─── Message reveal ───────────────────────────────────────────────────────
    showMessages() {
        // Show the initial clock at 5 seconds before fading in
        this.showElapsedTime(5);

        const messages = document.getElementById('messages');
        if (messages) {
            // Fade in the countdown area (3 seconds)
            this.fadeIn(messages, 3000, () => {
                // Once visible, start counting down 5 → 0
                let remaining = 5;
                const tick = setInterval(() => {
                    remaining--;
                    this.showElapsedTime(remaining);
                    if (remaining <= 0) {
                        clearInterval(tick);
                        this.showBirthdayNow();
                    }
                }, 1000);
            });
        }
    ...
    showLoveU() {
        ...
        if (loveu) this.fadeIn(loveu, 3500);
    }

    // ─── Utility: fade in ─────────────────────────────────────────────────────
    fadeIn(element, duration = 2000, callback = null) {
        if (!element) return;
